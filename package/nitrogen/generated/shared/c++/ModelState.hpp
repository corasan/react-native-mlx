...
namespace margelo::nitro::rnmlx {
  ...
  /**
   * A struct which can be represented as a JavaScript object (ModelState).
   */
  struct ModelState {
  public:
  ...
  public:
    explicit ModelState(bool isLoaded, bool isGenerating, std::string modelId, std::optional<std::string> error, std::string modelInfo): isLoaded(isLoaded), isGenerating(isGenerating), modelId(modelId), error(error), modelInfo(modelInfo) {}
  };
...
namespace margelo::nitro {
  ...
  // C++ ModelState <> JS ModelState (object)
  template <>
  struct JSIConverter<ModelState> final {
    static inline ModelState fromJSI(jsi::Runtime& runtime, const jsi::Value& arg) {
    ...
    static inline jsi::Value toJSI(jsi::Runtime& runtime, const ModelState& arg) {
